...
NUM_CLUSTERS = int(
    os.getenv("NUM_CLUSTERS", 10)
)  # Number of clusters to use for the composite method
UPSCALE_FACTOR = int(
    os.getenv("UPSCALE_FACTOR", 3)
)  # How much we upscale the image on each step of zooming in
FIRST_OCR_THRESHOLD = float(
    os.getenv("FIRST_OCR_THRESHOLD", 0.9)
)  # Threshold for the first OCR pass
SECOND_OCR_THRESHOLD = float(
    os.getenv("SECOND_OCR_THRESHOLD", 0.7)
)  # Threshold for the second OCR pass
...
class CompositeSelection(BaseModel):
    ...
    number: int = Field(
        ..., description="The number of the selected section of the composite"
    )
...
def find_coordinates(semdesk, description: str, text: str) -> dict:
    ...
    if search_text and len(search_text) > 3:
        ...
        best_matches = [
            ...
            if similarity_ratio(box["text"], search_text) >= FIRST_OCR_THRESHOLD
        ]
    ...
    region_of_interest, bounding_box = method(
        semdesk, starting_img, starting_img_path, description, click_hash, "region"
    )
    ...
    region_of_interest_path = os.path.join(
        semdesk.img_path, f"{click_hash}_region_of_interest.png"
    )
    ...
    if search_text:
        ...
        zoomed_region_of_interest = zoomed_region_of_interest.resize(
            (
                zoomed_region_of_interest.width * UPSCALE_FACTOR,
                zoomed_region_of_interest.height * UPSCALE_FACTOR,
            ),
            resample=0,
        )
        zoomed_region_of_interest_path = os.path.join(
            semdesk.img_path, f"{click_hash}_zoomed_region_of_interest.png"
        )
        ...
        best_matches = [
            box
            for box in ocr_results
            if similarity_ratio(box["text"], search_text) >= SECOND_OCR_THRESHOLD
        ]
        ...
        if len(best_matches) != 1:
            ...
            absolute_box = relative_box.to_absolute_with_upscale(
                bounding_boxes[-1], UPSCALE_FACTOR
            )
    ...
    region = region.resize(
        (region.width * UPSCALE_FACTOR, region.height * UPSCALE_FACTOR), resample=0
    )
    region_of_interest_path = os.path.join(
        semdesk.img_path, f"{click_hash}_region_of_interest_zoom_1.png"
    )
    region.save(region_of_interest_path)
    total_upscale *= UPSCALE_FACTOR
    new_region_of_interest, relative_bounding_box = method(
        semdesk, region, region_of_interest_path, description, click_hash, "zoom_1"
    )
    ...
    region_of_interest_path = os.path.join(
        semdesk.img_path, f"{click_hash}_region_of_interest_zoom_2.png"
    )
    region.save(region_of_interest_path)
    total_upscale *= UPSCALE_FACTOR
    last_region_of_interest, relative_bounding_box = method(
        semdesk, region, region_of_interest_path, description, click_hash, "zoom_2"
    )
    ...
    debug_img = _debug_image(starting_img.copy(), bounding_boxes, (x_mid, y_mid))
    semdesk.task.post_message(
        role="Clicker",
        msg="Final debug img",
        thread="debug",
...
def backup_find_coordinates(semdesk, description: str) -> dict:
    ...
    for i in [0, 1, 2]:
        ...
        region_of_interest_path = os.path.join(
            semdesk.img_path, f"{click_hash}_grid_region_{i}.png"
        )
        region.save(region_of_interest_path)
        total_upscale *= UPSCALE_FACTOR
        region_of_interest, relative_bounding_box = method(
            semdesk,
            region,
            region_of_interest_path,
            description,
            click_hash,
            "zoom_{i}",
        )
...
def run_composite(
    ...
    composite_path = os.path.join(
        semdesk.img_path, f"{click_hash}_composite_{postfix}.png"
    )
    ...
    region_of_interest = starting_image.crop(
